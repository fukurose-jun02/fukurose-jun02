# 👋 fukurose-jun02

HR×AIの実務とプロダクト企画をしながら、趣味で家族向けツールやブラウザゲームを作っています。
Claude / Claude Code を使った業務効率化や、子どもと遊べる小さなゲームづくりが中心です。

---

## 🎮 ブラウザゲーム

子ども向けの簡単なブラウザゲームを作っています。

| リポジトリ | 内容 |
|---|---|
| [princess-tower](https://github.com/fukurose-jun02/princess-tower) | 4〜6歳向け「おひめさまの塔のぼり」 |
| [sumikko_tsumitsumi](https://github.com/fukurose-jun02/sumikko_tsumitsumi) | すみっコをキャッチして積み上げるゲーム |
| [caterpillar-game](https://github.com/fukurose-jun02/caterpillar-game) | いもむしから逃げるゲーム |
| [bike-game](https://github.com/fukurose-jun02/bike-game) | 出前バイク便ゲーム「デリソバ・リバイバル」 |
| [candy-run](https://github.com/fukurose-jun02/candy-run) | キャンディを集めて脱出するゲーム |
| [snake-game](https://github.com/fukurose-jun02/snake-game) | パステルカラーのスネーク風ゲーム |
| [my-games](https://github.com/fukurose-jun02/my-games) | 自作ゲームのまとめページ |

---

## 👨‍👩‍👧 家族向けツール

家族の生活をちょっと便利にするツールを作っています。

| リポジトリ | 内容 |
|---|---|
| [fukuchan-app](https://github.com/fukurose-jun02/fukuchan-app) | 家族AIチャットボット「ふくちゃんトーク」フロントエンド |
| [fukuchan-knowledge](https://github.com/fukurose-jun02/fukuchan-knowledge) | 「ふくちゃんトーク」のナレッジ・プロンプト管理 🔒 |
| [youtube_kodomotoodekake](https://github.com/fukurose-jun02/youtube_kodomotoodekake) | 家族のおでかけ動画をまとめる記録サイト |
| [TubeAudio](https://github.com/fukurose-jun02/TubeAudio) | YouTube音声変換アプリ一式（iOS: SwiftUI + サーバー: Flask/yt-dlp） |

---

## 🩺 個人利用ツール

自分用に作っているツールです。

| リポジトリ | 内容 |
|---|---|
| [HealthDashboard](https://github.com/fukurose-jun02/HealthDashboard) | ローカルファーストの健康データダッシュボード（SwiftUI） 🔒 |

---

## 🤖 AI・自動化ツール

業務効率化やAI活用のための自作ツールです。

| リポジトリ | 内容 |
|---|---|
| [claude-skills](https://github.com/fukurose-jun02/claude-skills) | Claude Cowork用カスタムスキル集 🔒 |
| [gmail-line-notifier](https://github.com/fukurose-jun02/gmail-line-notifier) | GmailからLINEへ通知する自動化ツール 🔒 |
| [talent-management](https://github.com/fukurose-jun02/talent-management) | 社員情報・スキルを管理するタレントマネジメントシステム 🔒 |
| [competitor-intel](https://github.com/fukurose-jun02/competitor-intel) | HR SaaS/HCM競合のニュース・機能情報を蓄積するウォッチDB 🔒 |

---

## ⚙️ Claude Code関連

| リポジトリ | 内容 |
|---|---|
| [cc-secretary](https://github.com/fukurose-jun02/cc-secretary) | Claude Code用パーソナル秘書プラグイン 🍴 |
| [everything-claude-code](https://github.com/fukurose-jun02/everything-claude-code) | Claude Code運用ツール集 🍴 |

---

🔒 = 非公開リポジトリ　🍴 = フォーク

最終更新日: 2026-09-11
